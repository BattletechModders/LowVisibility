...
namespace LowVisibility.Helper
{
    public static class SensorLockHelper
    {
        ...
        // TODO: Dispatch structured log messages to asynchronous thread for SensorLockHelper
        public static float GetSensorsRange(AbstractActor source)
        {
            ...
            if (source.StatCollection.ContainsStatistic(ModStats.DisableSensors))
            {
                ...
                {
                    Mod.Log.Debug?.Write(
                        $"Returning minimum sensors range for {CombatantUtils.Label(source)} due to disabled sensors.");
                }
            ...
            {
                Mod.Log.Trace?.Write(
                    $"    rangeMulti: {rangeMulti} = rawRangeMulti: {rawRangeMulti} + sensorCheckRangeMulti: {ewState.GetSensorsRangeMulti()}");
            }
            ...
            {
                Mod.Log.Trace?.Write(
                    $"    rangeMod: {rangeMod} = rawRangeMod: {rawRangeMod} + sensorCheckRangeMulti: {ewState.GetSensorsRangeMulti()}");
            }
            ...
            {
                Mod.Log.Trace?.Write(
                    $"    sensorsRange: {sensorsRange} = baseRange: {ewState.GetSensorsBaseRange()} * rangeMult: {rangeMulti} + rangeMod: {rangeMod}");
            }
        ...
        public static SensorScanType CalculateSharedLock(ICombatant target, AbstractActor source, Vector3 previewPos)
        {
            ...
            foreach (AbstractActor actor in sensorSources)
            {
                ...
                if (actorScanType > scanType)
                {
                    ...
                    {
                        Mod.Log.Trace?.Write(
                            $"Increasing scanType to: ({actorScanType}) from actor:({CombatantUtils.Label(actor)}) ");
                    }
            ...
            if (previewPos != Vector3.zero)
            {
                ...
                if (sourceType > scanType)
                {
                    ...
                    {
                        Mod.Log.Trace?.Write(
                            $"Increasing scanType to: ({sourceType}) from source:({CombatantUtils.Label(source)}) ");
                    }
        ...
        public class SharedLockLogData() : AsyncMessageData
        {
            ...
            public SharedLockLogData(SensorScanType scanType, string target) : this()
            {
                _target = target;
                _scanType = scanType;
            }
            ...
            {
                return $"Shared lock to target:({_target}) is type: ({_scanType})";
            }
        }
        ...
        public static SensorScanType CalculateSensorLock(AbstractActor source, Vector3 sourcePos, ICombatant target, Vector3 targetPos)
        {
            ...
            if (source.GUID == target.GUID || source.Combat.HostilityMatrix.IsFriendly(source.TeamId, target.team.GUID))
            {
                ...
                {
                    Mod.Log.Trace?.Write(
                        $"  source:{CombatantUtils.Label(source)} is friendly to target:{CombatantUtils.Label(target)}. Forcing full visibility.");
                }
            ...
            if (source.IsDead || source.IsFlaggedForDeath)
            {
                ...
                {
                    Mod.Log.Trace?.Write(
                        $"  source:{CombatantUtils.Label(source)} is dead or dying. Forcing no visibility.");
                }
            ...
            if (target.IsDead || target.IsFlaggedForDeath)
            {
                ...
                {
                    Mod.Log.Trace?.Write(
                        $"  target:{CombatantUtils.Label(target)} is dead or dying. Forcing no sensor lock, vision based upon visibility.");
                }
            ...
            if (source.StatCollection.ContainsStatistic(ModStats.DisableSensors))
            {
                ...
                {
                    Mod.Log.Debug?.Write(
                        $"Sensors disabled for source: {CombatantUtils.Label(source)}, returning no info.");
                }
            ...
            {
                Mod.Log.Trace?.Write(
                    $"SensorLockHelper - source: {CombatantUtils.Label(source)} sensorRangeVsTarget: {sensorRangeVsTarget} vs distance: {distance}");
            }
            ...
            if (target is BattleTech.Building targetBuilding)
            {
                ...
                {
                    Mod.Log.Trace?.Write(
                        $"  target:{CombatantUtils.Label(targetBuilding)} is a building with lockState:{buildingLock}");
                }
            ...
            else if ((target as AbstractActor) != null)
            {
                ...
                if (distance > sensorRangeVsTarget)
                {
                    ...
                    {
                        Mod.Log.Trace?.Write(
                            $"  source:{CombatantUtils.Label(source)} is out of range, lock from Narc is:{narcLock}");
                ...
                else
                {
                    ...
                    {
                        Mod.Log.Trace?.Write(
                            $"SensorLockHelper - source:{CombatantUtils.Label(source)} has sensorLock:({sensorLock}) vs " +
            ...
            {
                if (Mod.Log.Info != null)
                {
                    Mod.Log.Info?.Write(
                        $"SensorLockHelper - fallthrough case for target: {CombatantUtils.Label(target)} with type: {target.GetType()}. Returning NoLock!");
                }
        ...
        private static SensorScanType CalculateSensorInfoLevel(AbstractActor source, ICombatant target)
        {
            ...
            {
                Mod.Log.Trace?.Write(
                    $"Calculating SensorInfo from source: ({CombatantUtils.Label(source)}) to target: ({CombatantUtils.Label(target)})");
            }
            ...
            if (sourceState.AdvancedSensorsMod() > 0)
            {
                ...
                {
                    Mod.Log.Trace?.Write(
                        $" == source has advanced sensors, detailsLevel = {positiveMods} + {sourceState.AdvancedSensorsMod()}");
                }
            ...
            if (sourceState.GetRawECMJammed() > 0)
            {
                ...
                {
                    Mod.Log.Trace?.Write(
                        $" == source is jammed by ECM, detailsLevel = {ecmNegativeMods} - {sourceState.GetRawECMJammed()}");
                }
            ...
            if (targetActor != null)
            {
                ...
                if (targetState.ECMDetailsMod(sourceState) > 0)
                {
                    ...
                    {
                        Mod.Log.Trace?.Write(
                            $" == target is shielded by ECM, detailsLevel = {ecmNegativeMods} - {targetState.ECMDetailsMod(sourceState)}");
                    }
                ...
                if (targetState.HasStealth())
                {
                    ...
                    {
                        Mod.Log.Trace?.Write(
                            $" == target has stealth, detailsLevel = {negativeMods} - {targetState.StealthDetailsMod()}");
                    }
                ...
                if (targetState.IsNarced(sourceState))
                {
                    ...
                    {
                        Mod.Log.Trace?.Write(
                            $" == target is NARC'd, detailsLevel = {positiveMods} + {targetState.NarcDetailsMod(sourceState)}");
                    }
                ...
                if (targetState.IsTagged(sourceState))
                {
                    ...
                    {
                        Mod.Log.Trace?.Write(
                            $" == target is tagged, detailsLevel = {positiveMods} + {targetState.TagDetailsMod(sourceState)}");
                    }
                ...
                if (targetState.PingedByProbeMod() != 0)
                {
                    ...
                    {
                        Mod.Log.Trace?.Write(
                            $" == target is pinged by probe, detailsLevel = {positiveMods} + {targetState.PingedByProbeMod()}");
                    }
            ...
            if (ecmNegativeMods < Mod.Config.Sensors.MaxECMDetailsPenalty)
            {
                ...
                {
                    Mod.Log.Trace?.Write(
                        $"  == negatives exceed cap, setting negative penalty to Sensors.MaxECMDetailsPenalty: {Mod.Config.Sensors.MaxECMDetailsPenalty}");
                }
            ...
            {
                Mod.Log.Trace?.Write(
                    $"  == detailsTotal: {detailLevel} = positiveMods: {positiveMods} + negativeMods: {negativeMods}");
            }
