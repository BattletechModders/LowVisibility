...
namespace LowVisibility.Helper {
    class SensorLockHelper {
        ...
        public static float GetSensorsRange(AbstractActor source) {
            ...
            {
                Mod.Log.Debug()?.Invoke($"Returning minimum sensors range for {CombatantUtils.Label(source)} due to disabled sensors.");
            ...
            Mod.Log.Trace()?.Invoke($"  == Sensors Range for for actor:{CombatantUtils.Label(source)}");
            ...
            Mod.Log.Trace()?.Invoke($"    rangeMulti: {rangeMulti} = rawRangeMulti: {rawRangeMulti} + sensorCheckRangeMulti: {ewState.GetSensorsRangeMulti()}");
            ...
            Mod.Log.Trace()?.Invoke($"    rangeMod: {rangeMod} = rawRangeMod: {rawRangeMod} + sensorCheckRangeMulti: {ewState.GetSensorsRangeMulti()}");

            float sensorsRange = ewState.GetSensorsBaseRange() * rangeMulti + rangeMod;
            Mod.Log.Trace()?.Invoke($"    sensorsRange: { sensorsRange} = baseRange: {ewState.GetSensorsBaseRange()} * rangeMult: {rangeMulti} + rangeMod: {rangeMod}");
        ...
        private static float GetAllTargetSignatureModifiers(AbstractActor target, EWState sourceState) {
            ...
            Mod.Log.Trace()?.Invoke($" Actor: {CombatantUtils.Label(target)} has signature: {targetSignature} = " +
        ...
        public static SensorScanType CalculateSharedLock(ICombatant target, AbstractActor source) {
            ...
            if (source == null) {
                ...
                Mod.Log.Trace()?.Invoke($"No primary lock source, assuming all allies.");
                ...
                Mod.Log.Trace()?.Invoke($"Actor:({CombatantUtils.Label(source)}) is primary lock source.");
                sensorSources.Add(source);
            }

            Mod.Log.Trace()?.Invoke($"Checking locks from {sensorSources.Count} sources.");
            foreach (AbstractActor actor in sensorSources) {
                ...
                if (actorScanType > scanType) {
                    Mod.Log.Trace()?.Invoke($"Increasing scanType to: ({actorScanType}) from source:({CombatantUtils.Label(actor)}) ");
                    scanType = actorScanType;
                }
            }

            Mod.Log.Trace()?.Invoke($"Shared lock to target:({CombatantUtils.Label(target)}) is type: ({scanType})");
            return scanType;
        }
        ...
        public static SensorScanType CalculateSensorLock(AbstractActor source, Vector3 sourcePos, ICombatant target, Vector3 targetPos) {
            ...
            if (source.GUID == target.GUID || source.Combat.HostilityMatrix.IsFriendly(source.TeamId, target.team.GUID)) {
                ...
                Mod.Log.Trace()?.Invoke($"  source:{CombatantUtils.Label(source)} is friendly to target:{CombatantUtils.Label(target)}. Forcing full visibility.");
            ...
            if (source.IsDead || source.IsFlaggedForDeath) {
                ...
                Mod.Log.Trace()?.Invoke($"  source:{CombatantUtils.Label(source)} is dead or dying. Forcing no visibility.");
            ...
            if (target.IsDead || target.IsFlaggedForDeath) {
                ...
                Mod.Log.Trace()?.Invoke($"  target:{CombatantUtils.Label(target)} is dead or dying. Forcing no sensor lock, vision based upon visibility.");
                return SensorScanType.NoInfo;
            }

            if (source.IsTeleportedOffScreen) {
                Mod.Log.Trace()?.Invoke($"  source as is teleported off screen. Skipping.");
                return SensorScanType.NoInfo;
            }

            if (source.StatCollection.ContainsStatistic(ModStats.DisableSensors))
            {
                Mod.Log.Debug()?.Invoke($"Sensors disabled for source: {CombatantUtils.Label(source)}, returning no info.");
                return SensorScanType.NoInfo;
            }
            ...
            Mod.Log.Trace()?.Invoke($"SensorLockHelper - source: {CombatantUtils.Label(source)} sensorRangeVsTarget: {sensorRangeVsTarget} vs distance: {distance}");
            if (target is BattleTech.Building targetBuilding) {
                ...
                Mod.Log.Trace()?.Invoke($"  target:{CombatantUtils.Label(targetBuilding)} is a building with lockState:{buildingLock}");
                return buildingLock;
                ...
                if (distance > sensorRangeVsTarget) {
                    ...
                    Mod.Log.Trace()?.Invoke($"  source:{CombatantUtils.Label(source)} is out of range, lock from Narc is:{narcLock}");
                    return narcLock;
                } else {
                    SensorScanType sensorLock = SensorScanType.NoInfo;
                    if (targetActor.IsTeleportedOffScreen) {
                        Mod.Log.Trace()?.Invoke($"  target is teleported off screen. Skipping.");
                    } else {
                    ...
                    Mod.Log.Trace()?.Invoke($"SensorLockHelper - source:{CombatantUtils.Label(source)} has sensorLock:({sensorLock}) vs " +
        ...
        private static SensorScanType CalculateSensorInfoLevel(AbstractActor source, ICombatant target) {
            Mod.Log.Trace()?.Invoke($"Calculating SensorInfo from source: ({CombatantUtils.Label(source)}) to target: ({CombatantUtils.Label(target)})");
            ...
            Mod.Log.Trace()?.Invoke($" == detailsLevel from EW check = {positiveMods}");

            // --- Source: Advanced Sensors
            if (sourceState.AdvancedSensorsMod() > 0) {
                Mod.Log.Trace()?.Invoke($" == source has advanced sensors, detailsLevel = {positiveMods} + {sourceState.AdvancedSensorsMod()}");
                positiveMods += sourceState.AdvancedSensorsMod();
            }

            // --- Source: ECM Jamming
            if (sourceState.GetRawECMJammed() > 0) {
                Mod.Log.Trace()?.Invoke($" == source is jammed by ECM, detailsLevel = {ecmNegativeMods} - {sourceState.GetRawECMJammed()}");
            ...
            if (targetActor != null) {
                ...
                if (targetState.ECMDetailsMod(sourceState) > 0) {
                    Mod.Log.Trace()?.Invoke($" == target is shielded by ECM, detailsLevel = {ecmNegativeMods} - {targetState.ECMDetailsMod(sourceState)}");
                    ecmNegativeMods -= targetState.ECMDetailsMod(sourceState);
                }

                // Stealth reduces sensor info
                if (targetState.HasStealth()) {
                    Mod.Log.Trace()?.Invoke($" == target has stealth, detailsLevel = {negativeMods} - {targetState.StealthDetailsMod()}");
                ...
                if (targetState.IsNarced(sourceState)) {
                    Mod.Log.Trace()?.Invoke($" == target is NARC'd, detailsLevel = {positiveMods} + {targetState.NarcDetailsMod(sourceState)}");
                ...
                if (targetState.IsTagged(sourceState)) {
                    Mod.Log.Trace()?.Invoke($" == target is tagged, detailsLevel = {positiveMods} + {targetState.TagDetailsMod(sourceState)}");
                ...
                if (targetState.PingedByProbeMod() != 0) {
                    Mod.Log.Trace()?.Invoke($" == target is pinged by probe, detailsLevel = {positiveMods} + {targetState.PingedByProbeMod()}");
            ...
            if (ecmNegativeMods < Mod.Config.Sensors.MaxECMDetailsPenalty) {
                Mod.Log.Trace()?.Invoke($"  == negatives exceed cap, setting negative penalty to Sensors.MaxECMDetailsPenalty: {Mod.Config.Sensors.MaxECMDetailsPenalty }");
                ecmNegativeMods = Mod.Config.Sensors.MaxECMDetailsPenalty; 
            }

            int detailLevel = positiveMods + negativeMods + ecmNegativeMods;
            Mod.Log.Trace()?.Invoke($"  == detailsTotal: {detailLevel} = positiveMods: {positiveMods} + negativeMods: {negativeMods}");

            SensorScanType sensorInfo = SensorScanTypeHelper.DetectionLevelForCheck(detailLevel);
            Mod.Log.Trace()?.Invoke($" == Calculated sensorInfo as: ({sensorInfo})");
