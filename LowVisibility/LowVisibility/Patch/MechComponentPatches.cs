...
namespace LowVisibility.Patch {
    ...
    public static class MechComponent_CancelCreatedEffects {
        ...
        public static void Prefix(MechComponent __instance, ref bool __state) {
            Mod.Log.Trace()?.Invoke("MC:CCE:pre entered.");

            // State indicates whether a stealth effect was found
            __state = false;

            Mod.Log.Debug()?.Invoke($" Cancelling effects from component: ({__instance.Name}) on actor: ({CombatantUtils.Label(__instance.parent)})");
            for (int i = 0; i < __instance.createdEffectIDs.Count; i++) {
                ...
                foreach (Effect effect in allEffectsWithID) {
                    if (effect.EffectData.effectType == EffectType.StatisticEffect && ModStats.IsStealthStat(effect.EffectData.statisticData.statName)) {
                        Mod.Log.Debug()?.Invoke($" -- Found stealth effect to cancel: ({effect.EffectData.Description.Id})");
                        __state = true;
                    }
                }
            }
        ...
        public static void Postfix(MechComponent __instance, bool __state) {
            Mod.Log.Trace()?.Invoke("MC:CCE:post entered.");

            if (__state) {
                Mod.Log.Debug()?.Invoke($" Stealth effect was cancelled, parent visibility needs refreshed.");
    ...
    //        Mod.Log.Trace()?.Invoke("MC:DC:post entered.");

    //        Mod.Log.Debug()?.Invoke($" Damaging component: ({__instance.Name}) on actor: ({CombatantUtils.Label(__instance.parent)} from hitInfo: {hitInfo.targetId}" +
