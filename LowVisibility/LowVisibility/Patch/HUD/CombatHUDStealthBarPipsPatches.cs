    ...
    class CombatHUDStealthBarPipsPatches {
        ...
        public static class CombatHUDStealthBarPips_ShowValue {
            public static void Postfix(CombatHUDStealthBarPips __instance, float current, float projected) {
                Mod.Log.Trace()?.Invoke("CHUDSBP:SV entered");

                Mod.Log.Trace()?.Invoke($"StealthBarPips incoming count is: {current} with projected: {projected}");
                ...
                Mod.Log.Trace()?.Invoke($"  selectedActor: ({CombatantUtils.Label(selectedActor)})");
                ...
                Mod.Log.Trace()?.Invoke($"  floorCurrent: {floorCurrent} floorLocked: {floorLocked} floorProjected: {floorProjected} remainder: {remainder}");

                Traverse pipsT = Traverse.Create(__instance).Property("Pips");
                List<Graphic> pips = pipsT.GetValue<List<Graphic>>();

                for (int i = 0; i < pips.Count; i++) {
                    Mod.Log.Trace()?.Invoke($"    -- pips graphic: {i} isEnabled: {pips[i].IsActive()}");
                }
            }
        }
        ...
        public static class MoveStatusPreview_DisplayPreviewStatus {
            ...
            public static void Prefix(MoveStatusPreview __instance, AbstractActor actor, Vector3 worldPos, MoveType moveType) {
                Mod.Log.Trace()?.Invoke("MSP:DPS entered.");
                ...
                if (actor.CurrentPosition != worldPos) {
                    ...
                    Mod.Log.Trace()?.Invoke($" position change for: ({CombatantUtils.Label(actor)}), moved {distance}m = {steps} steps");
